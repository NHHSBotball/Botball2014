...
#define MOTOR_POWER_LEVEL_MULTIPLIER 3

    int average(int i1, int i2, int i3);
    int arrAverage(int* arr, int length);
    int preformApproach(bool leftCube, int channelNum);
    ...
    int preformApproach(bool leftCube, int channelNum) {
        ...
        while (true) {
            ...
            if (possibleApproach) {
                ...
                while (that_end_time.tv_sec - that_start_time.tv_sec < 1) {//            !!!!!!!!!!!!!!!!!was 840
                    //gettimeofday(&that_end_time, NULL);
                }
                printf("xsum: %i\n", xSum);
                printf("xearly sum: %i\n", xEarlySum);
            ...
            if (abs(averageX) < DRIVE_TOLERANCE) {
                ...
                motor(BOTGUY_MOTOR_LEFT, (LEFT_WHEEL_POWER + averageX * MOTOR_POWER_LEVEL_MULTIPLIER) / 2);
                ...
                motor(BOTGUY_MOTOR_RIGHT, (RIGHT_WHEEL_POWER - averageX * MOTOR_POWER_LEVEL_MULTIPLIER) / 2);
            }
