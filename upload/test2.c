...
void runAutogeneratedCode2(bool slow) {
    ...
    while (getMS(that_end_time) - getMS(that_start_time) < 1090) { //adaptive mode

        if (digital(9)) {
            create_drive(-500, -1300);
        } else {
    ...
    msleep(1101); //time before putting up cube knock //Was 1341
    ...
    /*gettimeofday(&that_start_time, NULL);
    gettimeofday(&that_end_time, NULL);
    while (getMS(that_end_time) - getMS(that_start_time) < 801) {//            !!!!!!!!!!!!!!!!!was 840
